...
@inject()
export default class PassMixService {
  constructor(protected operationService: OperationService) {}
  async mix_first_step(data: any, auth: any) {
  ...
  async mix_second_step(data: any) {
    try {
  ...
  async mix_by_first_step_callback(response: any, status: string) {
    const ctx = await db.beginGlobalTransaction()
    try {
  ...
  async mix_by_second_step_callback(response: any, status: string) {
    const ctx = await db.beginGlobalTransaction()
    try {
      // Récupération de la transaction avec les relations nécessaires
      const { transaction, payment, user, wallet } = await this.operationService.data_web_hook(
  ...
  async data_forfait(request: any) {
    try {
      let response = await this.operationService.hub_service(
        `${process.env.API_AIRTIME_COUNTRY_URL}/${request.code}/operators?dataOnly=false&bundlesOnly=true&includeData=false&includeBundles=false`,
        'get',
        ''
      )
      return ResponseFormatter.create({
