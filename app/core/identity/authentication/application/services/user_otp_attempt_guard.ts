...
import UserOtpTemporarilyBlockedException from '#core/identity/authentication/domain/exceptions/user_otp_temporarily_blocked_exception'
...
export default class UserOtpAttemptGuard {
  ...
  private async autoBlockUser(user: User, ipAddress: string | null, count: number): Promise<void> {
    ...
    try {
      ...
      securityLog.error(
        'USER_OTP_GUARD_ACCOUNT_STATUS_SYNC_ERROR',
        { userId: user.id, error: (error as Error).message },
        'UserOtpAttemptGuard: failed to sync account status after auto-block'
      )
    }
